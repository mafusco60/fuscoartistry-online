<x-layout>
  ...
  <header>
    <h1 class="text-2xl text-center font-bold my-6 text-cyan-900">Messages</h1>
  </header>
    ...
    <table class="w-full table-auto rounded-sm">
     ...
     {{-- Display Messages --}}
      ...
      {{-- Display Image if it exists --}}
                  <p class="text-cyan-900 text-sm text-center font-semibold">
                  User Upload:
                  </p>
      ...
      {{-- Display the Uploaded Image if it exists --}}
        ...
        {{-- Download Image --}}    
                ...
                @else
                  <p class="text-cyan-500 font-semibold text-center text-sm p-4">
                  No Uploaded Image
                  </p>
                @endif
        ...
        {{-- Display Original Message Date and Time --}}
          ...
          {{-- Name --}}
                <h1 class="text-sm font-bold inline text-cyan-900 underline">Name:<h1 class="text-sm font-semibold text-black  inline">  {{ $message->name }}</h1><br>
                         
          {{-- Email --}}
                <a href="mailto:{{ $message->email }}" >
                  <h1 class="text-sm font-bold inline text-cyan-900 underline">Email:<h1 class="text-cyan-500 font-bold text-sm inline"> {{ $message->email }}</a>
                          
          {{-- Subject --}}
                <div>
                  <h1 class="text-sm font-bold inline text-cyan-900 underline">Subject:</h1>
          ...
          {{-- Artwork --}}
                ...
                <p class="text-sm font-bold  text-cyan-900 underline inline"> Artwork:</p>
                @if ($message->artwork_title != null)
                    {{$message->artwork_title}} 
                            
                @else
                  <p class="text-cyan-900 font-semibold pb-5 text-sm inline">No Artwork </p>
                @endif 

            {{-- User --}}
                      
                <h1 class="text-sm font-bold inline text-cyan-900 underline"><br>User:</h1>   
            ...
            {{-- Message body --}}
                <div>
                  <p class="text-sm font-bold inline text-cyan-900 underline ">Message:</p>
            ...
            {{-- Display the reply message if it exists --}}
                @if ($message->reply)
                  <div class="p-4 mt-4  border border-gray-300 text-md"
                          >
                    <p class="font-bold mb-4 text-cyan-500 underline">Reply Message:</p>
                    <p class="text-sm font-normal text-cyan-700 mb-4">{{ $message->reply }}
                    </p>
                ...
                @else
                  <div class= "text-lg">
                    <a href="{{route('messages.edit', $message->id)}}"
                      class="text-cyan-500 px-6 py-2 rounded-xl text-md">
                ...
                <td class="px-4 py-8 border-t border-b border-gray-300 text-lg"
                  ...
                  method="POST"
                    ...
                    class="text-red-400 py-2 rounded-xl">
                      ...
                      <i class="text-cyan-600 fa-solid fa-archive"
                      >
                      </i>
