...
<x-layout>
  ...
  <x-card class="py-2">
    <header>
      <h1 class="text-3xl text-center font-bold my-2 text-teal-900">Users</h1>
    </header>
        ...
        @unless ($users->isEmpty())
          @foreach ($users as $user)
            <tr class="border-gray-300">
              ...
              <td
                class="border-t border-b border-gray-300 text-teal-900 px-4"
              >
              ...
              <td class="border-t border-b border-gray-300">
                <x-card>
                  <div>
                    ...
                    <p class="text-sm text-teal-900 font-bold">
                      {{ $user->firstname }}
                      {{ $user->lastname }}: ID#
                      {{ $user->id }}
                    </p>

                    <div class="text-teal-500 font-bold text-sm">
                      {{-- Email Address --}}
                      <div>
                        <a
                      ...
                      @if ($user->subscribe == 1)
                        ...
                        <span class="inline text-teal-900">Yes</span>
                      @else
                        <i class="fas fa-times text-rose-500"></i>
                        <span class="inline text-teal-900">No</span>
                      @endif
                    </div>
                  </div>
                </x-card>
